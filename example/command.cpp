...
void test_command_chaining() {
    ...
    // 演示链式调用的强大之处
    auto cmd = Command::shell("find /etc -name '*.conf' | head -5");
    auto result = cmd.dir("/").capture_stdout().capture_stderr().timeout(std::chrono::seconds(10)).run();

    std::cout << "Command: " << cmd.commandline() << std::endl;
    std::cout << "Exit code: " << result.exit_code() << std::endl;
...
void test_while_async_run() {
    auto result = Command::shell("while true; do echo \"holder\"; sleep 1; done")
